function [ gammaTable1, gammaTable2, displayBaseline, displayRange, displayGamma, maxLevel, fullvals, inputValues] = CalibrateMonitorColorCal2PsychImaging(numMeasures, screenid, ColorCALIICDCPort, gamma)
% [gammaTable1, gammaTable2, displayBaseline, displayRange. displayGamma, maxLevel ] = CalibrateMonitorColorCal2([numMeasures=9],[screenid=max],[port=COM12], [gamma=[]])
%
...
%         ColorCALIIZeroCalibrate(ColorCALIICDCPort);
    ...
    try
        ...
        %% This is how you load a gamma table
        ...
        % load gamma table
        if ~isempty(gamma)
            if numel(gamma)==1 % assume simple camma correction
                PsychImaging('AddTask', 'FinalFormatting', 'DisplayColorCorrection', 'SimpleGamma');
                PsychColorCorrection('SetEncodingGamma', win, gamma, 'FinalFormatting');
            else
                % add lookup table color correction
                PsychColorCorrection('SetLookupTable', win, gamma, 'FinalFormatting');
            end
        end
