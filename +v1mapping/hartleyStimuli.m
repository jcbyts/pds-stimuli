...
%             p.trial.(sn).phi(p.trial.iFrame)=mod(2*pi*p.trial.(sn).n.tf*p.trial.iFrame*p.trial.display.ifi, 2*pi);
    ...
    case p.trial.pldaps.trialStates.trialSetup
        ...
        p.trial.(sn).hHart.ppd = 1; % TODO: Why is M ppd and ppd 1
    ...
    case p.trial.pldaps.trialStates.experimentPostOpenScreen
        ...
        p.trial.(sn).M   = p.trial.display.ppd; % scale factor to convert degrees to pixels
        freqs = sort([-2.^(0:(p.trial.(sn).nOctaves-1))*p.trial.(sn).Freq0 0 2.^(0:(p.trial.(sn).nOctaves-1))*p.trial.(sn).Freq0]);            
        p.trial.(sn).kxs = freqs;
        p.trial.(sn).kys = freqs;
    ...
    case p.trial.pldaps.trialStates.trialCleanUpandSave
        ...
        ix = p.trial.iFrame+1:p.trial.(sn).maxFrames;
        p.trial.(sn).kx(ix,:) = [];
        p.trial.(sn).ky(ix,:) = [];
        p.trial.(sn).on(ix,:) = [];
        p.trial.(sn).tf(ix,:) = [];
