classdef gaussianPyrNoise < handle
    ...
    methods
        % constructor
        function n=gaussianPyrNoise(p, varargin)
            ip=inputParser();
            ip.addParameter('N', 4)
            ...
            ip.addParameter('lifetime', 10)
            ip.addParameter('rng', RandStream('twister'))
            ip.parse(varargin{:})
            
            assert(isa(p, 'pldaps'), 'p must be a pldaps')
            n.ptr       = p.trial.display.ptr;
        ...
        function setup(n)
            ...
            % identically:
            n.mypars = repmat([n.contrast, n.sc, 1.0, 0]', 1, n.count);
