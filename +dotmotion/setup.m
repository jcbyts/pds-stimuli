...
p.trial.(sn).size			= 0.3; 		% dot size (diameter; deg)
p.trial.(sn).speed 			= 15.0; 	% dot speed (deg/sec) 
p.trial.(sn).numDots 		= 35;
...
p.trial.(sn).fixPointDim	= 2; % dimming of fixation point (0 invisible)
...
p.trial.(sn).cueApertureRadius 			= 1.8;
...
p.trial.(sn).choiceCueWidth 			= 6; 
p.trial.(sn).choiceErrWidth				= 12;
p.trial.(sn).choiceTargetDelay 			= 0;
p.trial.(sn).choiceWinMinRadius 		= 3.8;
p.trial.(sn).choiceWinMaxRadius 		= 7;
p.trial.(sn).choiceGracePeriod 			= 0.4;   % grace period (aka flight time; sec)
...
% --- Fixation
p.trial.(sn).hFix(1) 	= stimuli.fixation(p.trial.display.overlayptr);
p.trial.(sn).hFix(2) 	= stimuli.fixation(p.trial.display.overlayptr);
...
p.trial.(sn).hFbk  	 = stimuli.circles(p.trial.display.overlayptr); % big bold circle
